...
def load_process_text(file):
    ...
    The CSV should contain 'title' and 'text' columns that
    are created in the 'cleaning_util.py' file. This function
    tokenizes and lemmatizes text, as well as returning total
    text length.

    Args:
        file: A CSV containing 'title' and 'text' columns.
...
def class_word_counts(df, class_):
    ...
    Args:
        class_: A string that corresponds to a grouping
          variable to be filtered in df.
    
    Returns:
        A collections.Counter object containing the classes
        word counts.
...
def class_count_dict(df, classes, top_words = 10):
    """Constructs a dictionary of class keys and word count values.
    
    Args:
        classes: A list that contains grouping variables to be 
          filtered in df.
        top_words: An integer (default = 10) indicating the amount of 
          words to be returned; words are in descending frequency 
          (e.g., 1 = the most common word for that class).
...
class D2V:
    ...
    def model_train(self, 
        ...
        if self.model_type == 'dbow':
            ...
            model = Doc2Vec(vector_size = vector_size, min_count = min_count, epochs = epochs, workers = workers)
